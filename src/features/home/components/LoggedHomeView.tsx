...
export default function LoggedHomeView() {
  ...
  return (
    <div className="w-full h-full px-4 pb-8 pt-8 sm:px-6 lg:px-8 flex flex-col">
      <div className="mx-auto w-full max-w-[1400px] flex-1 flex flex-col gap-5">
        <section className="min-w-0 flex-1 flex flex-col">
          <div className="mb-5 flex items-center justify-between gap-4">
            <div>
              <p className="text-xs uppercase tracking-[0.2em] text-white/50">¡Bienvenido de nuevo!</p>
              <h1 className="text-2xl font-bold text-white sm:text-3xl">{user?.name || "Cinéfilo"}</h1>
            </div>
          ...
          <article className="relative overflow-hidden rounded-2xl border border-red-500/20 bg-gradient-to-br from-zinc-900 via-zinc-900 to-black p-5">
            ...
            <div className="relative z-10 max-w-xl space-y-3">
              ...
              <p className="text-sm text-white/70">
                Reanuda tu watch party con sincronización en tiempo real y chat integrado para no perderte ninguna escena.
              </p>
              <div className="flex gap-3">
                <button className="rounded-full bg-white px-5 py-2 text-sm font-semibold text-black">Ver ahora</button>
                <button className="rounded-full border border-white/25 px-5 py-2 text-sm font-semibold text-white/90">Detalles</button>
              </div>
            </div>
          </article>
          ...
          <section className="mt-6">
            <div className="mb-3 flex items-center justify-between">
              <h3 className="text-xl font-semibold text-white">Tendencias</h3>
              <button className="text-sm text-white/65 hover:text-white">Ver todo</button>
            </div>
          ...
          <section className="mt-6 grid gap-4 xl:grid-cols-[2fr_1fr]">
            <div>
              <h3 className="mb-3 text-xl font-semibold text-white">Recomendadas para ti</h3>
            ...
            <aside className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
              <h3 className="mb-3 text-lg font-semibold text-white">Seguir viendo</h3>
              <div className="space-y-3">
                {continueWatchingItems.map((item) => (
                  <article key={item.id} className="rounded-xl border border-white/10 bg-black/35 p-3">
                    <h4 className="text-sm font-semibold text-white">{item.title}</h4>
                    <p className="mt-1 text-xs text-white/60">{item.episode || "Reproduciendo ahora"}</p>
