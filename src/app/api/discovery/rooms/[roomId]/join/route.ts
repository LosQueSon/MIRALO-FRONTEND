...
export async function POST(request: NextRequest, context: { params: Promise<{ roomId: string }> }) {
  ...
  const requestBody = (await request.json().catch(() => null)) as { accessCode?: string } | null

  const joinResponse = await fetch(`${backendBaseUrl}/rooms/${roomId}/users/${userId}/join`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    body: JSON.stringify({ accessCode: requestBody?.accessCode ?? "" }),
  })

  const payload = await joinResponse.json().catch(() => null)
  return NextResponse.json(payload, { status: joinResponse.status })
}
